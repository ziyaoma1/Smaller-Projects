...
class Citizen:
        ...
        if self.cid == cid:
            ...
            for sub in self.get_direct_subordinates():
                if sub.cid == cid:
                    ...
                    lst.append(sub.get_citizen(cid))
        for item in lst:
            if item is not None:
                return item
        return None
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        for sub in self.get_direct_subordinates():
            if sub.get_direct_subordinates():
                lst = merge(lst, sub.get_all_subordinates())
        return lst
        # Note: This method must call itself recursively
        # Hints:
    ...
    def get_highest_rated_subordinate(self) -> Citizen:
        ...
        lst1 = []
        for item in lst:
            highest[item.rating] = item
            lst1.append(item.rating)
        return highest[max(lst1)]
...
class Society:
    ...
    def change_citizen_type(self, cid: int,
        ...
        if isinstance(citizen, DistrictLeader):
            citizen1 = Citizen(citizen.cid, citizen.manufacturer,
                               citizen.model_year, citizen.job, citizen.rating)
        else:
            citizen1 = DistrictLeader(citizen.cid, citizen.manufacturer,
                                      citizen.model_year, citizen.job,
                                      citizen.rating, district_name)
        n = 0
        if citizen == self._head:
            n += 1
        self.delete_citizen(citizen.cid)
        if sup is not None:
            sup.remove_subordinate(citizen)
            sup.add_subordinate(citizen1)
        citizen1.set_superior(sup)
        citizen.set_superior(None)
        for subs in sub:
            subs.become_subordinate_to(citizen1)
        if n == 1:
            self._head = citizen1
        return citizen1
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        sup1 = superior.get_direct_subordinates()
        sup2 = citizen.get_direct_subordinates()
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        citizen = self.get_citizen(cid)
        if isinstance(citizen, DistrictLeader) \
                or citizen.get_superior() is None:
            return None
        elif citizen.rating > citizen.get_superior().rating:
            citizen = self._swap_up(citizen)
        if not isinstance(citizen,
                          DistrictLeader) and citizen.get_superior() \
                is not None and citizen.rating > \
                citizen.get_superior().rating:
            return self.promote_citizen(cid)
        return None
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        Precondition: There is a Citizen with the cid <cid> in this Society.
        """
        citizen = self.get_head().get_citizen(cid)
        if citizen == self._head and citizen.get_direct_subordinates():
            new = citizen.get_highest_rated_subordinate()
            new.set_superior(None)
            self.set_head(new)
            for subs in citizen.get_direct_subordinates():
                if subs != new:
                    new.add_subordinate(subs)
                    subs.set_superior(new)
            return None
        elif citizen == self._head and not citizen.get_direct_subordinates():
            self._head = None
            return None
        else:
            pass
        ...
        if citizen.get_direct_subordinates():
            for item in citizen.get_direct_subordinates():
                item.become_subordinate_to(superior)
        citizen.become_subordinate_to(None)
        return None
